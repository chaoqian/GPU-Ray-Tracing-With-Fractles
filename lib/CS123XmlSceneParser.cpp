...
bool CS123XmlSceneParser::parsePrimitive(const QDomElement &prim, CS123SceneNode* node)
{
    ...
    mat.mu.r = -0.278; mat.mu.g = -0.479; mat.mu.b = -0.231; mat.mu.a = 0.235;
    ...
    else if (primType == "julia") primitive->type = PRIMITIVE_JULIA;
    else if (primType == "mandelbulb") primitive->type = PRIMITIVE_MANDELBULB;
    else if (primType == "mandelbox") primitive->type = PRIMITIVE_MANDELBOX;
    ...
    while (!childNode.isNull())
    {
        ...
        else if (e.tagName() == "mu")
        {
            if (!parseQuadruple(e, mat.mu.r, mat.mu.g, mat.mu.b, mat.mu.a, "x", "y", "z", "w"))
            {
                PARSE_ERROR(e);
                return false;
            }
