...
vec3<T> vec3<T>::reflectVector(const vec3 &normal) const {
    ...
    const vec3<T> & V = (*this);
    const vec3<T>   N = normal.getNormalized();
    return V - static_cast<T>(2)*V.dot(N)*N;
...
vec3<T> vec3<T>::refractVector(const vec3 &normal, T in, T out) const {
    ...
    const double cos = N.dot(V);
    const double sin = SQ(n) * (1.0 - SQ(cos));
    if (sin > 1.0)
    {   //error
        return vec3<T>();
    }
    return n*V - (n*cos + std::sqrt(1.0 - sin))*N;
};
