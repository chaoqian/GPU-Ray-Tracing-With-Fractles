...
//@{-----------------------------------------------------------------
    ...
    inline vec4_T getReflection(const vec4_T& normal) const {
        const vec4_T V = getNormalized();
        const vec4_T N = normal.getNormalized();
        return vec4_T(V - 2.0*V.dot(N)*N).getNormalized();
    }

    inline vec4_T getRefraction(const vec4_T& normal, T in, T out) const {
        const vec4_T V = getNormalized();
        const vec4_T N = normal.getNormalized();
        const double n = in / out;
        const double cos1 = N.dot(-V);
        const double cos2 = std::sqrt(1.0-SQ(n)*(1.0-SQ(cos1)));
        if (cos1<0.0)
        {
            return n*V - (n*cos1 - cos2)*N;
        }
        return n*V + (n*cos1 - cos2)*N;
    }
...
struct mat4 {
    ...
    static inline mat4<T> diag(T x, T y, T z, T w) {return diag(vec4<T>(x,y,z,w));}
    static mat4<T> diag(vec4<T> const& v);
...
typedef mat4<char>      Matrix4x4b;
typedef mat4<int>       Matrix4x4i;
typedef mat4<unsigned>  Matrix4x4u;
typedef mat4<float>     Matrix4x4f;
typedef mat4<double>    Matrix4x4d;
